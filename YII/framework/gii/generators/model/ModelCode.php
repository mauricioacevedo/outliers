...
class ModelCode extends CCodeModel
{
	...
	public $extraRules="array('fechacreacion','default', 'value'=>new CDbExpression('NOW()'), 'setOnEmpty'=>false, 'on'=>'insert'),
			...
			array('modificadopor','default', 'value'=>Yii::app()->user->name, 'setOnEmpty'=>false, 'on'=>'update'),";	
	/*public $extraRules="array('create_date','default', 'value'=>new CDbExpression('NOW()'), 'setOnEmpty'=>false, 'on'=>'insert'),
			...
			array('modified_by','default', 'value'=>Yii::app()->user->name, 'setOnEmpty'=>false, 'on'=>'update'),";*/
	...
	public function rules()
	{
		return array_merge(parent::rules(), array(
			array('tablePrefix, baseClass, tableName, moduleName, modelClass, modelPath, connectionId,extraRules', 'filter', 'filter'=>'trim'),
	...
	{  
		...
		list($this->relations,$this->relations2)=$this->generateRelations();
		foreach($tables as $table)
		{
			...
			$params=array(
				...
				'relations2'=>isset($this->relations2[$className]) ? $this->relations2[$className] : array(),
				'connectionId'=>$this->connectionId,
			);
                        if($this->moduleName == '0'){
                            $modelPath = $this->modelPath;
                        }else{
                            $modelPath = "application.modules.".$this->moduleName.".models";
                        }
	...
	public function generateRules($table)
	{
		...
		foreach($table->columns as $column)
		{
			...
			$filterTrim[]=$column->name;
			
                        if($column->isForeignKey){                       
                            $filterTrim[] = 'foreign_'.$column->name;
                        }
		...
		$rules[]="array('".implode(', ',$filterTrim)."', 'filter', 'filter'=>'trim')";		
		if($required!==array())
		...
		foreach(Yii::app()->{$this->connectionId}->schema->getTables($schemaName) as $table)
		{
			...
			if ($this->isRelationTable($table))
			{
				...
				$relations[$className0][$relationName]="array(self::MANY_MANY, '$className1', '$unprefixedTableName($pks[0], $pks[1])')";
                                $relations2[$className0][$relationName] = array('refTable'=>$table0, 'field' => $pks[0]);
				...
				$relations2[$className1][$relationName] = array('refTable'=>$table0, 'field' => $pks[0]);
			}
			else
			{
				...
				foreach ($table->foreignKeys as $fkName => $fkEntry)
				{
					...
					$relations2[$className][$relationName]=array('refTable'=>$refTable, 'field' => $fkName);
					...
					$relations2[$refClassName][$relationName] = array('refTable'=>$refTable, 'field' => $fkName);
				}
			}
		}// print_r($relations2); exit;
		return array($relations,$relations2);
	}
